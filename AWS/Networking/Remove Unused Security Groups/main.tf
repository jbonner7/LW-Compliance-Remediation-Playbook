#### Remediation action start -> the remediation action calls AWS SSM Automation document execution -> AWS SSM assumes the IAM role -> the automation deletes the unused security group by ID discovered by AWS Config
